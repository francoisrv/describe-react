...
export type HasTypeProps =
  ...
  | {
      not?: boolean
      type: boolean
      which: React.ReactElement<IsProps<TypeIdentifier>, typeof Is>
    }

export type HasTextProps = ({ not?: true } | { no?: true }) & {
  text: string | true
...
export type HasPropsProps =
  | (({ not?: true } | { no?: true }) & { properties: true })
  | (({ not?: true } | { no?: true }) & {
      exact?: true
...
interface ChildrenProps {
  children: true | TypeIdentifier
}
interface ChildProps {
  child: true | TypeIdentifier
}

type ManyChildrenProps = ChildrenProps &
...
interface SiblingsProps {
  siblings: true | TypeIdentifier
}
interface SiblingProps {
  sibling: true | TypeIdentifier
}

type ManySiblingsProps = SiblingsProps &
  NotProps & { which?: Which<TypeIdentifier> } & (
    | {}
    | { exactly: number }
    | { at: true; least: number }
